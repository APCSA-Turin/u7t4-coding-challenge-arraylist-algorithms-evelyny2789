...
public class Main{
   ...
   */
    ...
    public static ArrayList<String> parseSentence(String sentence){
        ...
        while(str.indexOf(" ") != -1){
            ind = str.indexOf(" "); 
            parsed.add(str.substring(0, ind));
    ...
    public static ArrayList<String> moveBWords(ArrayList<String> wordList){
        ...
        for (int i = 0; i < wordList.size(); i++){
            if (wordList.get(i).toLowerCase().indexOf("b") != -1){
                wordList.add(bInd, wordList.remove(i)); 
                bInd++; 
                i--; 
            }
        }
   ...
   */
    public static ArrayList<Integer> modes(int[] numList){
        ...
        boolean unique; 
        int ind = 0; 
        uniqueList.add(numList[0]); 
        amount.add(1);
        for (int i = 1; i < numList.length; i++){
            unique = true; 
            for (int k = 0; k < uniqueList.size(); k++){
                if (uniqueList.get(k) == numList[i]){
                    unique = false; 
                    ind = k; 
                }
            }
            if (unique){
                ...
                amount.set(ind, amount.get(ind) + 1); 
            }
        }
        int max = 1; 
        int maxCount = 0; 
        ...
        for (int i = 0; i < amount.size(); i++){
            if (amount.get(i) > max){
                max = amount.get(i); 
                maxCount = 1; 
            } else if (max == amount.get(i)){
                maxCount++; 
            }
        }
        if (max != 1 && maxCount != amount.size()){
            for (int i = 0; i < amount.size(); i++){
                if (amount.get(i) == max){
                    modes.add(uniqueList.get(i)); 
                }
            }
        }
        return modes;
